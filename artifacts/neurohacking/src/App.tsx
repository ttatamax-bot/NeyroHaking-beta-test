...
const TECH_MESSAGES: { text: string; highlight: string[] }[] = [
  ...
  { text: 'Визуализация укрепляет мотивацию.', highlight: ['T2'] },
  { text: 'Медитация развивает осознанность.', highlight: ['T3'] },
...
function OnboardingTutorial() {
  ...
  if (onboardingStep === 2) {
    return <KnowledgeChatPanel onDone={() => { updateState({ onboardingHighlight: [], userState: 'active', onboardingComplete: true }); setLocation('/academy'); }} />;
  }
  return null;
}
...
function AppLogic() {
  ...
  useEffect(() => {
    const check = () => {
      ...
      const lastTechDate = storeRef.current.todayTechniquesDate
        ? getAppDayKey(new Date(storeRef.current.todayTechniquesDate))
        : null;
      const isNewDay = !lastTechDate || lastTechDate !== todayKey;

      if (isNewDay) {
        // Reset streak if missed days
        const streakUpdates: Partial<AppState> = {};
        if (storeRef.current.lastCompletedDate) {
          const todayStart = getAppDayStart(now);
          const yesterdayStart = new Date(todayStart);
          yesterdayStart.setDate(yesterdayStart.getDate() - 1);
          const lastCompleted = getAppDayStart(new Date(storeRef.current.lastCompletedDate));
