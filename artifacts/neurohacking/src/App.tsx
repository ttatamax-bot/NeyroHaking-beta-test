...
// v0.0.1: багфиксы + аналитика + обновленное описание
...
// Coaching sequences shown to user after onboarding
const COACHING_SEQUENCES: {
  ...
  },
  {
    ...
    condition: s =>
      s.keys >= 5 &&
      s.readArticles.includes('A1') &&
      !s.unlockedArticles.includes('A2'),
    messages: [
      'У тебя достаточно ключей для покупки статьи',
      'Прочитай про постановку целей — это очень важно!',
    ],
  },
  {
    id: 'after_goals',
...
function TechChatPanel({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    updateRef.current({ onboardingHighlight: msg?.highlight ?? [] });
  }, [idx]);

  const advance = () => {
    if (isLast) {
      updateRef.current({ onboardingHighlight: [] });
      onDone();
    } else {
      setIdx(i => i + 1);
    }
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[35] cursor-pointer"
        onClick={advance}
...
function OnboardingTutorial() {
  ...
  if (onboardingStep === 0) {
    return (
      <TechChatPanel
        onDone={() => {
          updateState({ onboardingStep: 1, onboardingHighlight: [] });
          setLocation('/path');
        }}
      />
    );
  ...
  if (onboardingStep === 2) {
    return (
      <KnowledgeChatPanel
        onDone={() => {
          updateState({ onboardingHighlight: [], userState: 'email' });
...
// Coaching bubble shown after onboarding at key moments
function CoachingBubble() {
  ...
  // Find triggered coaching message
  useEffect(() => {
    ...
    for (const seq of COACHING_SEQUENCES) {
      if (!shown.includes(seq.id) && seq.condition(storeSnap)) {
        const timer = setTimeout(() => {
          setActive({ id: seq.id, messages: seq.messages, idx: 0 });
          setVisible(true);
        }, 900);
  ...
  const handleTap = () => {
    ...
    if (nextIdx < active.messages.length) {
      ...
      updateRef.current(prev => ({
        coachingShown: [...(prev.coachingShown ?? []), id],
      }));
      setVisible(false);
      setActive(null);
    }
...
function AppLogic() {
  const { userState, updateState } = useAppStore();
...
function DayDoneOverlay() {
  ...
  // Don't block the email screen during onboarding
  if (location.startsWith('/onboarding/email')) return null;

  return (
    <div className="fixed inset-0 z-[50] flex flex-col items-center justify-center text-primary overflow-hidden"
      style={APP_BG}
    >
...
function AppLayout({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="min-h-[100dvh] w-full max-w-[390px] mx-auto text-primary relative overflow-hidden flex flex-col"
      style={APP_BG}
    >
      <AppLogic />
      <TopBar />
