...
For passwordless web auth, use the standard resources exposed through `useClerk().client.signUp` and `.signIn`, not the signal-based Future resources. Use `useAuth().isLoaded` for a reactive ready check; `useClerk().loaded` can leave a custom form stuck in its loading state. Email-code flows use create/prepare/attempt; magic-link flows use `createEmailLinkFlow().startEmailLinkFlow()` and the Clerk instance's `handleEmailLinkVerification()`. Custom sign-up forms must mount Clerk's `#clerk-captcha` placeholder before `signUp.create()`.
