...
function AppLogic() {
  ...
  // Day reset — at 5am after sleep completed.
  // T1–T5 carry over from the previous day (stay done), only T6 (sleep) resets.
  useEffect(() => {
    const check = () => {
      if (userState === 'dayDone') {
        ...
        if (now.getHours() >= 5) {
          updateState(prev => ({
            userState: 'active',
            todayTechniques: {
              T1: prev.todayTechniques.T1,
              T2: prev.todayTechniques.T2,
              T3: prev.todayTechniques.T3,
              T4: prev.todayTechniques.T4,
              T5: prev.todayTechniques.T5,
              T6: false,
            },
          }));
        }
